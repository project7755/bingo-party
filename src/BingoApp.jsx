...
/* Produce a deterministic order of cell positions (excluding center) based on player seed */
function positionOrder(seed){
  const pos=[]; for(let i=0;i<GRID*GRID;i++) if(i!==CENTER) pos.push(i);
  return shuffleSeeded(pos, `pos-${seed}`);
}
...
export default function BingoPartyCloud(){
  ...
  const [words,setWords] = useState([]); // array of {id, room_id, text, created_at}
  const [calls,setCalls] = useState([]); // array of {id, room_id, word, created_at}
  const [errorMsg,setErrorMsg] = useState("");
  const unsubRef = useRef(null);

  // Init: ensure room, fetch initial data, subscribe to realtime (deduped)
  useEffect(()=>{
    let alive = true;
    ...
    async function initCloud(){
      ...
      if (!r) {
        ...
        if (ins.error) { setErrorMsg(ins.error.message); return; }
        r = ins.data;
      }
      if (!alive) return;
      setRoom(r);
      ...
      if (!w.error) setWords(w.data||[]); else setErrorMsg(w.error.message||"");
      if (!c.error) setCalls(c.data||[]); else setErrorMsg(c.error.message||"");
      ...
      ch.on("postgres_changes",
        ...
        (payload)=> setWords(prev => {
          if (prev.some(x => x.id === payload.new.id)) return prev; // dedupe
      ...
      ch.subscribe();
      unsubRef.current = () => { try { supabase.removeChannel(ch); } catch(_) {} };
    }
  ...
  /* ========= Actions (with optimistic UI + dedupe) ========= */
  async function addWord(text){
    ...
    setWords(prev => [...prev, tmpRow]); // optimistic
    ...
    if (error) {
      setWords(prev => prev.filter(r => r.id !== tmpId)); // rollback
    ...
    setWords(prev => {
      ...
      if (withoutTmp.some(r => r.id === data.id)) return withoutTmp; // already added via realtime
  ...
  async function callWord(wordText){
    ...
    setCalls(prev => [...prev, tmpRow]); // optimistic
  ...
  /* ========= Derived values ========= */
  const wordPool = useMemo(()=> words.map(w=>w.text), [words]);
  const calledArr = useMemo(()=> calls.map(c=>c.word), [calls]);
  ...
  const boardWords = useMemo(()=>{
    ...
    cells[CENTER] = "FREE";
    const limit = Math.min(wordPool.length, order.length); // up to 24
    ...
    <div className="bp-wrap">
      ...
      <div className="bp-layout">
        ...
        <div className="bp-card">
          ...
          {/* Scaling wrapper prevents horizontal scroll on small phones */}
          <div className="bp-board-wrap">
            <BoardGrid words={boardWords} marked={marked}/>
          </div>

          <div style={{fontSize:12,color:"#666",marginTop:8}}>
            Room: <code>{roomCode}</code> · Cloud · Different <em>seed</em> = different layout.
          </div>
        </div>
      </div>
    </div>
